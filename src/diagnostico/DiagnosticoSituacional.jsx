...
import { criarDiagnosticoId, deveBuscarDiagnosticoRemoto, filtrarDiagnosticosPorEstado, filtrosConsultaDiagnosticos, mergeRespostasPorAtualizacao } from "./diagnosticoSync";
...
function friendlyPersistError(error) {
  ...
  if(code.includes("permission-denied"))return "Seu perfil não tem permissão para salvar neste projeto.";
  if(/unavailable|network-request-failed/.test(code))return "Sem conexão com o Firestore. O rascunho local continua protegido.";
  if(code.includes("resource-exhausted"))return "O diagnóstico excedeu o limite aceito pelo Firestore.";
  return "Não foi possível salvar o diagnóstico no Firestore. O rascunho local continua protegido.";
}
...
export default function DiagnosticoSituacional({ auth, db, dark, onToggleTheme, onBack, projects={}, projectGroups={} }) {
  ...
  const [filtroDiagnosticos,setFiltroDiagnosticos]=useState("ativos");
  const [acaoDiagnosticoId,setAcaoDiagnosticoId]=useState(null);
  ...
  const relatorio=useMemo(()=>montarRelatorioDiagnostico({catalogo,itens,respostas,contexto:projetoContexto||{},diagnosticoId:projetoContexto?.diagnosticoId||null,estado:projetoContexto?.estado||"rascunho"}),[catalogo,itens,respostas,projetoContexto]);
  ...
  const diagnosticosAtivos=filtrarDiagnosticosPorEstado(diagnosticos,"ativos");
  const diagnosticosArquivados=filtrarDiagnosticosPorEstado(diagnosticos,"arquivados");
  const diagnosticosVisiveis=filtroDiagnosticos==="arquivados"?diagnosticosArquivados:diagnosticosAtivos;
  ...
  const salvarDiagnostico=async(estado="rascunho")=>{
    ...
    try{
      ...
      if(diagnosticoExistente){
        ...
        remotas=remoto.exists()?(remoto.data().respostas||{}):{};
      }
    ...
    }catch(e){console.error("[diagnostico] falha ao salvar",e?.code||e);setPersistError(friendlyPersistError(e));}
    finally{setPersistBusy(false);}
  };
  const escolherProjeto=async(ctx)=>{
    setProjetoContexto(ctx);
    setDiagnosticoSelecionado(ctx?.tipo==="novo");
    setFiltroDiagnosticos("ativos");
    setPersistError("");setPersistSuccess("");
    setRascunhoLocal(null);
    setRespostas({});
  ...
  const alterarEstadoDiagnostico=async(d,acao)=>{
    const arquivar=acao!=="restaurar";
    if(acao==="remover"&&!window.confirm("Excluir este diagnóstico da lista ativa? Ele será preservado em Arquivados e poderá ser restaurado."))return;
    setAcaoDiagnosticoId(d.id);setPersistError("");setPersistSuccess("");
    try{
      const agora=new Date().toISOString();
      const alteracoes={estado:arquivar?"arquivado":"rascunho",arquivadoEm:arquivar?agora:null,atualizadoEm:agora};
      await updateDoc(doc(db,"diagnosticos",projetoContexto.projetoRef,"itens",d.id),alteracoes);
      setDiagnosticos(lista=>lista.map(item=>item.id===d.id?{...item,...alteracoes}:item));
      setPersistSuccess(arquivar?(acao==="remover"?"Diagnóstico removido da lista ativa e preservado em Arquivados.":"Diagnóstico arquivado."):"Diagnóstico restaurado para a lista ativa.");
    }catch(e){console.error("[diagnostico] falha ao organizar",e?.code||e);setPersistError(friendlyPersistError(e));}
    finally{setAcaoDiagnosticoId(null);}
  };
  ...
  if(projetoContexto.tipo==="existente"&&!diagnosticoSelecionado&&(diagnosticos.length>0||temRascunhoLocal))return <main style={{...styles.page,background:c.bg}}><section style={{...styles.listCard,background:c.card,borderColor:c.border,color:c.text}}>
    <h1 style={{...styles.title,color:c.text}}>Diagnósticos — {projetoContexto.projetoRef}</h1>
    <p style={{...styles.muted,color:c.muted}}>Organize os registros sem apagar o histórico.</p>
    {temRascunhoLocal&&<><button onClick={continuarRascunhoLocal} style={{...styles.localDraftButton,color:c.text}}><strong>Continuar rascunho deste aparelho</strong><span>{rascunhoLocal.savedAt?new Date(rascunhoLocal.savedAt).toLocaleString("pt-BR"):"sem data"} · {calculateProgress(itens,rascunhoLocal.respostas||{}).percentual}%</span></button><p style={{...styles.muted,color:"#f59e0b"}}>Protegido localmente. Abra e salve no Firestore antes de iniciar outro.</p></>}
    <div style={styles.filterTabs}><button onClick={()=>setFiltroDiagnosticos("ativos")} style={{...styles.filterTab,background:filtroDiagnosticos==="ativos"?"#2563eb":"transparent",color:filtroDiagnosticos==="ativos"?"#fff":c.muted,borderColor:c.border}}>Ativos ({diagnosticosAtivos.length})</button><button onClick={()=>setFiltroDiagnosticos("arquivados")} style={{...styles.filterTab,background:filtroDiagnosticos==="arquivados"?"#475569":"transparent",color:filtroDiagnosticos==="arquivados"?"#fff":c.muted,borderColor:c.border}}>Arquivados ({diagnosticosArquivados.length})</button></div>
    {diagnosticosVisiveis.length===0&&<p style={{...styles.emptyState,color:c.muted,borderColor:c.border}}>Nenhum diagnóstico nesta lista.</p>}
    {diagnosticosVisiveis.map(d=><div key={d.id} style={{...styles.diagnosticRow,borderColor:c.border}}><button disabled={temRascunhoLocal||d.estado==="arquivado"} onClick={()=>abrirDiagnostico(d)} style={{...styles.diagnosticOpen,color:c.text,opacity:(temRascunhoLocal||d.estado==="arquivado") ? .55 : 1}}><strong>{d.estado||"rascunho"} · {d.atualizadoEm?new Date(d.atualizadoEm).toLocaleString("pt-BR"):"sem data"}</strong><span>{calculateProgress(itens,d.respostas||{}).percentual}% preenchido</span></button><div style={styles.rowActions}>{d.estado==="arquivado"?<button disabled={acaoDiagnosticoId===d.id} onClick={()=>alterarEstadoDiagnostico(d,"restaurar")} style={{...styles.smallAction,color:"#22c55e",borderColor:"#22c55e66"}}>Restaurar</button>:<><button disabled={acaoDiagnosticoId===d.id} onClick={()=>alterarEstadoDiagnostico(d,"arquivar")} style={{...styles.smallAction,color:"#f59e0b",borderColor:"#f59e0b66"}}>Arquivar</button><button disabled={acaoDiagnosticoId===d.id} onClick={()=>alterarEstadoDiagnostico(d,"remover")} style={{...styles.smallAction,color:"#ef4444",borderColor:"#ef444466"}}>Excluir da lista</button></>}</div></div>)}
    {persistError&&<p role="alert" style={styles.error}>{persistError}</p>}{persistSuccess&&<p role="status" style={styles.success}>{persistSuccess}</p>}
    <button disabled={temRascunhoLocal} onClick={novoDiagnostico} style={{...styles.primary,opacity:temRascunhoLocal ? .55 : 1}}>＋ Novo diagnóstico</button><button onClick={()=>setProjetoContexto(null)} style={{...styles.secondary,color:c.muted,borderColor:c.border}}>Trocar projeto</button>
  </section></main>;
  ...
  return <main style={{...styles.page,background:c.bg,color:c.text}}><div style={styles.shell}>
    ...
    <section ref={contentTopRef} style={styles.content}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button onClick={()=>salvarDiagnostico("rascunho")} disabled={persistBusy} style={styles.primary}>{persistBusy?"Salvando…":"Salvar diagnóstico"}</button><button onClick={()=>salvarDiagnostico("arquivado")} disabled={persistBusy} style={styles.secondary}>Arquivar</button><button onClick={()=>setProjetoContexto(null)} style={styles.secondary}>Trocar projeto</button></div>{persistError&&<p role="alert" style={styles.error}>{persistError}</p>}{persistSuccess&&<p role="status" style={styles.success}>{persistSuccess}</p>}
      <div style={{...styles.exportCard,background:c.card,borderColor:c.border}}><strong style={{fontSize:13}}>Entregáveis do diagnóstico</strong><span style={{...styles.muted,color:c.muted}}>O rascunho permanece editável após a exportação.</span><div style={styles.exportActions}><button onClick={()=>setShowResumo(v=>!v)} style={{...styles.secondary,color:c.text,borderColor:c.border}}>{showResumo?"Ocultar resumo":"Ver resumo"}</button><button onClick={exportarPdf} disabled={exportBusy} style={styles.primary}>{exportBusy?"Gerando…":"Baixar PDF"}</button><button onClick={()=>baixarTextoDiagnostico(relatorio)} style={{...styles.secondary,color:c.text,borderColor:c.border}}>Baixar TXT</button></div>{exportError&&<p role="alert" style={styles.error}>{exportError}</p>}{showResumo&&<div style={{...styles.summaryBox,background:c.alt,borderColor:c.border}}><div><strong>{relatorio.percentual}%</strong><span> progresso</span></div><div><strong>{relatorio.achados}</strong><span> achados para análise</span></div><div style={styles.summaryCounts}>{Object.entries(STATUS_UI).map(([status,info])=><span key={status} style={{color:info.color}}>{info.short}: {relatorio.contagens[status]||0}</span>)}</div></div>}</div>
      <button onClick={clearDraft} style={{...styles.clearButton,color:c.muted}}>Limpar rascunho local</button><p style={{...styles.disclaimer,color:c.muted}}>Diagnóstico vinculado a {projetoContexto.rotuloLivre||projetoContexto.projetoRef}; catálogo permanece somente leitura.</p>
    </section>
  </div></main>;
}

const styles={
  listCard:{alignSelf:"center",width:"calc(100% - 24px)",maxWidth:520,boxSizing:"border-box",border:"1px solid",borderRadius:18,padding:"22px 18px",display:"grid",gap:12},
  filterTabs:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7},
  filterTab:{border:"1px solid",borderRadius:9,padding:9,fontSize:11,fontWeight:800,cursor:"pointer"},
  diagnosticRow:{border:"1px solid",borderRadius:11,padding:10,display:"grid",gap:8},
  diagnosticOpen:{border:0,background:"transparent",padding:0,display:"grid",gap:3,textAlign:"left",cursor:"pointer",fontSize:11},
  rowActions:{display:"flex",gap:7,flexWrap:"wrap"},
  smallAction:{border:"1px solid",borderRadius:8,background:"transparent",padding:"7px 9px",fontSize:10,fontWeight:800,cursor:"pointer"},
  emptyState:{border:"1px dashed",borderRadius:9,padding:15,textAlign:"center"},
